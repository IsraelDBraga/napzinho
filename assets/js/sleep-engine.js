/* ---- sleep engine (dates + classification) ---- */
...
function totalSleepMinsCalendarDay(ymd){const nightSegs=new Set(nightSegmentsOnWakeDay(ymd).map(e=>e.id));let sum=0;entries.forEach(e=>{if(e.type!=='sleep'||!e.end||!e.durationMins)return;if(sleepIsNight(e)){if(nightSegs.has(e.id))sum+=e.durationMins;}else{if(e.date===ymd)sum+=e.durationMins;}});const a=getActiveSleep();if(a&&!a.end){const st=parseTimeOnDate(a.start,a.date);const cur=Math.max(0,Math.round((now()-st)/60000));if(sleepIsNight(a)){const wd=fmtDateSV(sleepEndDate({...a,end:fmtTime(now())}));if(wd===ymd)sum+=cur;}else if(a.date===ymd)sum+=cur;}return sum;}
function calendarDayEntries(ymd){const nightSegs=new Set(nightSegmentsOnWakeDay(ymd).map(e=>e.id));return entries.filter(e=>{if(!e||!e.date)return false;if(e.type!=='sleep')return e.date===ymd;if(!e.end){const act=getActiveSleep();if(!act||act.id!==e.id)return false;if(!sleepIsNight(act))return act.date===ymd;return fmtDateSV(sleepEndDate({...act,end:fmtTime(now())}))===ymd;}if(sleepIsNight(e))return nightSegs.has(e.id);return e.date===ymd;});}
/** Groups completed night segments into sessions: consecutive sleeps with gap ≤120 min belong together. */
function nightSessionsAll(){const all=entries.filter(e=>e.type==='sleep'&&sleepIsNight(e)&&e.end).sort((a,b)=>sleepEndDate(a)-sleepEndDate(b));if(!all.length)return[];const sessions=[[all[0]]];for(let i=1;i<all.length;i++){const prev=all[i-1],cur=all[i];const tE=sleepEndDate(prev),tS=sleepStartDate(cur);const rawGap=tE&&tS?Math.round((tS-tE)/60000):Infinity;if(rawGap<=120){sessions[sessions.length-1].push(cur);}else{sessions.push([cur]);}}return sessions;}
/** Returns all segments that belong to the night session whose last segment wakes on ymd. */
function nightSegmentsOnWakeDay(ymd){const sess=nightSessionsAll().find(s=>sleepWakeCalendarDay(s[s.length-1])===ymd);return sess?[...sess].sort((a,b)=>sleepEndDate(a)-sleepEndDate(b)):[];}
...
// Expose helpers for other modules (explicit is safer after refactors).
window.dayBoundaryMins=dayBoundaryMins;
window.nightStartMinsVal=nightStartMinsVal;
window.isNightContextForClock=isNightContextForClock;
window.setSleepKind=setSleepKind;
window.sleepIsNight=sleepIsNight;
