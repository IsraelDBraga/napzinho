/* ---- sleep classification helpers ---- */
function dayBoundaryMins(){const v=isValidTime(cfg?.dayBoundary)?cfg.dayBoundary:'06:00';return timeToMins(v);}
function nightStartMinsVal(){const v=isValidTime(cfg?.nightStart)?cfg.nightStart:'18:00';return timeToMins(v);}
function isNightContextForClock(hhmm){const m=timeToMins(hhmm||fmtTime(now()));const db=dayBoundaryMins(),ns=nightStartMinsVal();return (m>=ns)||(m<db);}
function sleepIsNight(e){return !!(e&&e.type==='sleep'&&String(e.sleepKind||e.subtype||'').toLowerCase()==='night');}
function nightMinDurationByAge(){const m=babyAgeMonths();if(m<=2)return 40;if(m<=4)return 55;if(m<=6)return 70;if(m<=9)return 85;if(m<=12)return 95;return 105;}
function setSleepKind(e){
  if(!e||e.type!=='sleep'||!isValidTime(e.start)){return e;}
  const st=timeToMins(e.start),db=dayBoundaryMins(),ns=nightStartMinsVal();
  const endV=isValidTime(e.end)?e.end:null;
  const en=endV?timeToMins(endV):null;
  const inNightStart=(st>=ns)||(st<db);
  let kind='nap';
  if(inNightStart){
    kind='night';
  }else if(en!=null){
    const crossesMidnight=en<st;
    const touchesNight=(st<ns&&(crossesMidnight||en>=ns));
    const dur=Number.isFinite(e.durationMins)?e.durationMins:0;
    if(touchesNight&&dur>=nightMinDurationByAge())kind='night';
  }
  e.sleepKind=kind;
  e.subtype=kind;
  return e;
