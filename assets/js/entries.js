...
function babyWoke(){if(profileNeedsSetup()){showToast('Cadastre o bebê primeiro');return;}const active=getActiveSleep();if(!active){showToast('Nenhum sono em andamento');return;}const t=fmtTime(now());const idx=entries.findIndex(e=>e.id===active.id);entries[idx].end=t;const sm=parseTime(entries[idx].start),em=parseTime(t);if(em<sm)em.setDate(em.getDate()+1);entries[idx].durationMins=Math.round((em-sm)/60000);setSleepKind(entries[idx]);save();showToast('Acordou às '+t+' · '+fmtDur(entries[idx].durationMins));renderHome();}
...
function orbitCtaClick(){
  const cta = $('orbit-cta');
  const action = ((cta && cta.dataset && cta.dataset.action) || '').toLowerCase();
  ...
  if (action === 'sleep-manual' || action === 'manual') {
    openLog('sleep-manual');
    return;
  }

  if (action === 'log') {
    const logType = ((cta && cta.dataset && cta.dataset.logType) || '').toLowerCase();

    if (logType === 'feed' || logType === 'diaper' || logType === 'sleep-manual') {
      openLog(logType);
      return;
    }
  }

  // Fallback correto para o CTA orbital:
  // se está dormindo, acorda; se não está dormindo, inicia sono.
  if (typeof getActiveSleep === 'function' && getActiveSleep()) {
    babyWoke();
  } else {
    babySlept();
  }
