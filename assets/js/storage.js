...
function safeSetItem(key,value){
  try{localStorage.setItem(key,value);return true;}
    ...
    console.error('[storage] setItem failed',key,msg);
    if(/quota|exceeded|storage/i.test(msg)&&typeof showToast==='function'){
      showToast('Armazenamento cheio. Exporte backup e limpe espaço.');
    }
    return false;
  }
}
...
function persistAll(){const p=getCurrentProfile();const blob=loadProfilesBlob()||{};blob[p.id]={entries:JSON.parse(JSON.stringify(entries||[])),cfg:normalizeCfgForImport(cfg||{})};safeSetItem(PROFILE_KEY,JSON.stringify(blob));}
